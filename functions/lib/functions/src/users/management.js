...
app.post("/users", async (req, res) => {
    try {
        const validatedData = health_schema_1.userProfileSchema.parse(req.body);
        const userData = {
            ...validatedData,
...
app.get("/users/:userId/summary", async (req, res) => {
    try {
        ...
        const [assessmentsCount, metricsCount, goalsCount, activitiesCount, reportsCount] = await Promise.all([
...
async function initializeUserHealthData(userId) {
...
async function getLatestDocument(collectionPath) {
    try {
...
async function calculateAccountAge(userId) {
    try {
        const userDoc = await db.doc(`users/${userId}`).get();
...
exports.userManagement = (0, https_1.onRequest)({ region: "us-central1" }, app);

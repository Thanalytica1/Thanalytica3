...
const health_schema_1 = require("@shared/health-schema");
...
app.get("/metrics/:userId", async (req, res) => {
    try {
        ...
        if (startDate) {
            query = query.where("recordedAt", ">=", startDate);
        }
        if (endDate) {
            query = query.where("recordedAt", "<=", endDate);
        }
        query = query.orderBy("recordedAt", "desc").limit(Number(limit));
...
app.post("/metrics/:userId", async (req, res) => {
    try {
        const { userId } = req.params;
        // Validate the data
        const validatedData = health_schema_1.healthMetricSchema.parse({
            ...req.body,
            userId
        });
        const metricData = {
...
function calculateHealthTrends(metrics, metricType) {
    if (metrics.length === 0) {
        return {
...
function generateHealthInsights(metricType, trend, improvement, average) {
    const insights = [];
    if (!metricType || !trend)
        return insights;
    switch (metricType) {
...
exports.healthMetrics = (0, https_1.onRequest)({ region: "us-central1" }, app);

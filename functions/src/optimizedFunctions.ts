...
import { metricsEngine } from "../../server/services/metricsCalculationEngine";
...
lightweightApp.use(compression());

// Connection pooling for Firestore (reuse connections)
let firestoreConnectionPool: any = null;
...
lightweightApp.get('/api/dashboard/:userId', async (req, res) => {
  ...
  try {
    ...
    const dashboardCache = await cacheService.getDashboardCache(userId);
    ...
    metricsEngine.calculateAndCacheUserMetrics(userId).catch(error => {
...
lightweightApp.get('/api/metrics/:userId/:timeframe', async (req, res) => {
  ...
  try {
    ...
    switch (timeframe) {
      case 'daily':
        cacheData = await cacheService.getDailyMetrics(userId);
        break;
      case 'weekly':
        cacheData = await cacheService.getWeeklyMetrics(userId);
        break;
      case 'monthly':
        cacheData = await cacheService.getMonthlyMetrics(userId);
        break;
      case 'lifetime':
        cacheData = await cacheService.getLifetimeMetrics(userId);
        break;
    }
...
export const apiLightweight: HttpsFunction = onRequest({
...
export const dailyMetricsProcessor = onSchedule({
  ...
  try {
    ...
    for (let i = 0; i < userIds.length; i += chunkSize) {
      ...
      await Promise.allSettled(
        chunk.map(userId => 
          metricsEngine.calculateAndCacheUserMetrics(userId)
            .catch(error => {
...
export const weeklyCorrelationAnalysis = onSchedule({
      ...
      await Promise.allSettled(
        chunk.map(async userId => {
          try {
            ...
            await cacheService.invalidateCache(userId, ['monthlyMetrics']);
            
            // Trigger recalculation
            await metricsEngine.calculateAndCacheUserMetrics(userId);
          } catch (error) {
...
export const cacheCleanup = onSchedule({
  ...
  try {
    ...
    if (stats.oversizedDocuments > 0) {
      ...
      // Compact them
      await Promise.allSettled(
        oversizedSnapshot.docs.map(doc => 
          cacheService.invalidateCache(doc.id, ['lifetimeMetrics'])
            .catch(error => {
...
export const onAssessmentCreated = onDocumentCreated({
  ...
  try {
    ...
    await cacheService.invalidateCache(assessment.userId, [
      'dashboardCache',
      'lifetimeMetrics'
    ]);
...
export const onWearableDataCreated = onDocumentCreated({
  ...
  try {
    ...
    await cacheService.invalidateCache(wearableData.userId, [
      'dailyMetrics',
      'dashboardCache'
    ]);

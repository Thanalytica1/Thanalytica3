...
app.get("/assessments/:userId", async (req, res) => {
  try {
    ...
    query = query.orderBy("createdAt", "desc").limit(Number(limit)) as any;
...
async function calculateAssessmentScores(userId: string, assessmentId: string, assessmentData: any) {
  try {
...
function calculateSleepScore(answer: any): number {
  if (typeof answer === 'number') {
    if (answer >= 7 && answer <= 9) return 90;
    if (answer >= 6 && answer < 7) return 70;
    if (answer >= 5 && answer < 6) return 50;
    return 30;
  }
  return 50; // Default for non-numeric answers
}

function calculateExerciseScore(answer: any): number {
  if (typeof answer === 'string') {
    const freq = answer.toLowerCase();
    if (freq.includes('daily') || freq.includes('5-7')) return 95;
    if (freq.includes('4-5') || freq.includes('most')) return 85;
    if (freq.includes('2-3') || freq.includes('few')) return 70;
    if (freq.includes('1-2') || freq.includes('rarely')) return 50;
    return 30;
  }
  return 50;
}

function calculateNutritionScore(answer: any): number {
  if (typeof answer === 'string') {
    const diet = answer.toLowerCase();
    if (diet.includes('mediterranean') || diet.includes('plant')) return 90;
    if (diet.includes('balanced') || diet.includes('healthy')) return 80;
    if (diet.includes('standard') || diet.includes('mixed')) return 60;
    return 40;
  }
  return 50;
}

function calculateStressScore(answer: any): number {
  if (typeof answer === 'number') {
...
function generateAssessmentInsights(scores: Record<string, number>, insights: any) {
...
export const assessments = onRequest({ region: "us-central1" }, app);
...
function initializeFirebase() {
    ...
    if (!process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
        const error = new Error("FIREBASE_SERVICE_ACCOUNT_KEY environment variable is required but not set.\n" +
    ...
    try {
        ...
        const error = new Error(`Invalid FIREBASE_SERVICE_ACCOUNT_KEY format: ${parseError instanceof Error ? parseError.message : 'Unknown format error'}\n` +
    ...
    if ((0, app_1.getApps)().length === 0) {
        try {
            ...
            const error = new Error(`Firebase initialization failed: ${initError instanceof Error ? initError.message : 'Unknown error'}\n` +
...
async function testFirestoreConnection(db) {
    try {
        ...
        if (process.env.NODE_ENV === 'production') {
            throw new Error('Firestore connection failed. Application cannot start without database access. ' +
 ...
 */
async function retryFirestoreOperation(operation, maxRetries = 3, baseDelay = 1000) {
    let lastError;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            const isRetryableError = lastError.message.includes('UNAVAILABLE') ||
